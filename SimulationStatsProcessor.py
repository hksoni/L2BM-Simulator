...
def plot_metrics_for_all_the_algos(file_name, dest_dir_loca, lbl_plot_data_dict_ls, metrics_index, np_group_size,
    ...
    for lbl_plot_data_dict in lbl_plot_data_dict_ls:
        ...
        for l in sorted_order:
            ...
            l_s.append(l)
            # eb = plt.plot(np_group_size, metric[:,0], color=color_dict[l], marker=marker.next(), label=l)
...
def plot_average_nomalized_datapoints_for_different_groups(run_type, links_stat_dirs, plot_save_dir, labels,
                                                           group_start,
                                                           group_stop, group_int, group_runs, color_dict=None):
    ref_label = 'DST-PL'
    np_lbl_plot_avg_data_dict, np_lbl_plot_std_data_dict, np_lbl_plot_max_data_dict = \
        compute_average_nomalized_datapoints_for_different_groups(run_type, links_stat_dirs, plot_save_dir, labels,
                                                                  ...
                                                                  group_runs, color_dict)
    np_group_size = np.arange(int(group_start), int(group_stop) + 1, int(group_int))
    plot_normalized_data(np_group_size, color_dict, ref_label, [np_lbl_plot_avg_data_dict], [np_lbl_plot_std_data_dict],
                         [np_lbl_plot_max_data_dict])


def plot_normalized_data(np_group_size, color_dict, lbl_reference_model, np_lbl_plot_avg_data_dict_ls,
    ...
    plot_normalized_metrics_for_all_the_algos('norm-avg-lu', plot_save_dir,
                                              np_lbl_plot_avg_data_dict_ls, None, np_group_size, color_dict, xlable,
    ...
    plot_normalized_metrics_for_all_the_algos('norm-max-lu', plot_save_dir,
                                              np_lbl_plot_max_data_dict_ls, None, np_group_size, color_dict, xlable,
    ...
    loc = 'upper right'
    plot_normalized_metrics_for_all_the_algos('norm-stddev-lu', plot_save_dir,
                                              np_lbl_plot_std_data_dict_ls,
                                              None, np_group_size, color_dict, xlable, 'Normalized Stddev utilization',
...
def plot_normalized_metrics_for_all_the_algos(file_name, dest_dir, lbl_plot_dicts, metrics_index, np_group_size,
                                              colors, xlable, ylable, title, x1=None, x2=None,
                                              xticks_ls=None, y1=None, y2=None,
                                              yticks_ls=None, loc='upper left', reference_skip=None):
    plt.clf()
    fig = plt.figure()
    ax = fig.add_subplot(111)
    label_add = ['Sim', 'Testbed']
    linestyles = ['-', '--']
    ...
    eb_l = []
    l_s = []
    for lbl_plot_dict in lbl_plot_dicts:
        eb_l = []
        l_s = []
        for lbl, data in lbl_plot_dict.iteritems():
            ...
            eb = plt.errorbar(np_group_size, data[:, 0], color=colors[lbl], fmt=linestyles[i], yerr=err * data[:, 1],
                              label=l)
            eb[-1][0].set_linestyle(linestyles[i])
            eb_l.append(eb)
